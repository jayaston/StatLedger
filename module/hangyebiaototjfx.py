...
def load_datacode():
    data_code = pd.read_excel(r'../数据表/行业报表指标与台账指标编码对应表.xlsx',sheet_name=2,dtype={'台账编码':np.object,'对应报表中指标名称':np.object})
    
    return data_code
   
#    set(data_code[ data_code.duplicated('对应报表中指标名称')]['对应报表中指标名称'])
def load_deptcode():
    dept_code = pd.read_excel(r'../数据表/行业部门与台账部门编码对应表.xlsx',sheet_name=0,dtype={'部门':np.object,'编码':np.object})
